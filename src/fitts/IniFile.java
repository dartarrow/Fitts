...
public class IniFile {
	...
	{	//Load the ini file
		...
		try 
		{	File iniFile = new File(new File(".").getCanonicalPath()+"/fitts.ini");
			...
			if(i>=2) id = buf[2];
			if(i>=5) weightList = new Double(buf[5]).intValue();
			if(i>=8) targetList = new Double(buf[8]).intValue();
			if(i>=11) typeList = new Double(buf[11]).intValue();
			if(i>=14) keyTypeList = new Double(buf[14]).intValue();
			if(i>=17) diamList = new Double(buf[17]).intValue();
			if(i>=20) inType = new Double(buf[20]).intValue();
			if(i>=31) 
			{ 	for(int q=0;q<5;q++)
				{	coords[q]=new Point(new Double(buf[22+q*2]).intValue(),
										new Double(buf[23+q*2]).intValue());
